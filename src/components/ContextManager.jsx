...
export const ContextManager = ({ layers }) => {
	...
	let removeLayerid2ContextId = useAppStore((state) => state.removeLayerid2ContextId);
    let setLayerid2ContextId = useAppStore((state) => state.setLayerid2ContextId);
    let getLayerid2ContextId = useAppStore((state) => state.getLayerid2ContextId);
    ...
    function onLayerChange() {
		for(let layer of app.activeDocument.layers){
			...
			let contextId = getLayerid2ContextId(layer.id)
			if(!contextId){
				contextId = CreateAILayerContextId(layer)
				setAILayerContext(contextId, CreateAILayerContext(layer))
				setLayerid2ContextId(layer.id, contextId)
			}else{
				// lets get this layers context
				let context = getAILayerContext(contextId)
				
				// does this context have any active layers?
				let isActive = hasActiveLayers(context)

				// remove the connection between the layer and the context
		...
		try{
			...
			if(Object.keys(layerAIContexts).length > 0){
				for(let layer of app.activeDocument.layers){
					...
					let layerContext = getAILayerContext(CreateAILayerContextId(layer))
	...
	function CreateLayersFromContexts() {
		...
		return (
			...
			{contextList && contextList.map((context) => 
				<ContextItem key={context.id} layerContext={context}></ContextItem>
			)}
			</>
		)
