const openJpg = async () => {
    const jpg = await fs
        .getPluginFolder()
        .then((folder) => folder.getEntry('img/cat.jpg'));
...
const pickJpgSavingToken = async () => {
    const userSelected = await fs.getFileForOpening({
...
const openJpgFromToken = async () => {
    let f = await fs.getEntryForPersistentToken(
        localStorage.getItem('alwaysAvailable')
    );
    if (!f) return;
...
const getFolderOpenJpgs = async () => {
    ...
    if (!userSelected) return;
    const jpgs = await userSelected
        .getEntries()
        .then((entries) =>
            entries.filter(
                (entry) =>
                    entry.isFile && entry.name.slice(-3).toLowerCase() === 'jpg'
            )
        );
...
const getJpgSaveRenamed = async () => {
    const userSelected = await fs.getFolder();
    if (!userSelected) return;
    const jpg = await userSelected
        .getEntries()
        .then(
            (entries) =>
                entries.filter(
                    (entry) => entry.isFile && entry.name.slice(-3) === 'jpg'
                )[0]
        );
    await executeAsModal(
        async () => {
    ...
    let newJpg = await userSelected.createEntry(
        '' + jpg.name.slice(0, -4) + '-DUP.jpg'
    );
